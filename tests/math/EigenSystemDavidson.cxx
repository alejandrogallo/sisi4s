...
public:
  ...
  Vector<complex, D> right_apply(const Vector<complex, D> &v) const {
    Vector<complex, D> w;
    for (int i(0); i < D; ++i) {
      for (int k(0); k < D; ++k) { w[i] += A(i, k) * v[k]; }
    }
    return w;
  }
...
public:
  DiagonalDominantPreconditioner(const LinearMap<D> &f) {
    for (int i(0); i < D; ++i) { diagonal[i] = f.A(i, i); }
  }
  ...
  Vector<complex, D> getCorrection(const complex lambda,
                                   const Vector<complex, D> &residuum) const {
    Vector<complex, D> w;
    // compute ((lambda * id - Diag(diagonal))^-1) . residuum
    for (int k(0); k < D; ++k) { w[k] = residuum[k] / (lambda - diagonal[k]); }
    return w;
  }
...
TEST_CASE("EigenSystemDavidson", "[math]") {
  ...
  EigenSystemDavidson<Vector<complex, N>>
      eigenSystemDavidson(f, 4, DiagonalDominantPreconditioner<N>(f), 1E-8, 64);

  for (unsigned int k(0); k < eigenSystemDavidson.getEigenValues().size();
       ++k) {
    ...
    LOG(0, "LapackGeneralEigenSystem")
        << "EigenValue[" << k << "]=" << eigenSystem.getEigenValues()[k]
        << std::endl;
    LOG(0, "LapackGeneralEigenSystem")
        << "EigenValueDavison[" << k
