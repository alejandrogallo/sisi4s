...
*** Names of settings and algorithms

=sisi4s= is not sensitive to the naming convention
used in the inputs, thus an input name such as
=maxIterations= will be equivalent to the following
inputs
...
#+begin_src yaml
- name: Uccsd Amplitudes from Coulomb Integrals
  in:
    max-iterations: 32
  out:
    ...
...
=sisi4s= converts all inputs into a normal form which is
case insensitive and removes any spaces, =_= or =-= symbols.
...
will simply ignore =Uccsd Amplitudes from Coulomb Integrals=.
