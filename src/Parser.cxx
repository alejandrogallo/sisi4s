...
Algorithm *InputFileParser<InputFileFormat::CC4S>::parseAlgorithm() {
  ...
  if (!algorithm) {
    ...
    throw new DetailedException(sStream.str(),
                                stream.getSource(),
                                line,
                                column);
  }
...
Argument
InputFileParser<InputFileFormat::CC4S>::parseImplicitlyNamedArgument() {
...
Argument
InputFileParser<InputFileFormat::CC4S>::parseExplicitlyNamedArgument() {
  // TODO: store debug info for later reference in case of errors
...
std::string InputFileParser<InputFileFormat::CC4S>::parseData() {
  ...
  if (isalpha(character)) {
    ...
    throw new DetailedException("Constant or symbol expression expected",
                                stream.getSource(),
                                stream.getLine(),
                                stream.getColumn());
  }
...
TextData *InputFileParser<InputFileFormat::CC4S>::parseText() {
  ...
  stream.get();
  char c;
  while ((c = stream.peek()) > 0 && c != '"') { sStream.put(stream.get()); }
  expectCharacter('"');
...
NumericData *InputFileParser<InputFileFormat::CC4S>::parseNumber() {
  ...
  if (!isdigit(stream.peek())) {
    throw new DetailedException("Digit expected",
                                stream.getSource(),
                                stream.getLine(),
                                stream.getColumn());
  }
...
RealData *
InputFileParser<InputFileFormat::CC4S>::parseReal(const int64_t sign,
                                                  const int64_t integerPart) {
...
void InputFileParser<InputFileFormat::CC4S>::expectCharacter(
    char const expectedCharacter) {
  char character(stream.peek());
  if (character != expectedCharacter) {
    std::stringstream sStream;
    sStream << "Expected '" << expectedCharacter << "', got '" << character
            << "'";
    throw new DetailedException(sStream.str(),
                                stream.getSource(),
                                stream.getLine(),
