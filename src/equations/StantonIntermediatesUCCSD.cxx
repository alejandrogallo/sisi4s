...
void StantonIntermediatesUCCSD<F>::checkInputs() {
  ...
  std::map<std::string, Tensor<F> *> inputTensors{{"Tai", Tai},
                                                  {"Tabij", Tabij},
                                                  {"Fij", Fij},
                                                  {"Fab", Fab},
                                                  {"Vabcd", Vabcd},
  ...
  for (const auto &entry : inputTensors)
    if (!entry.second) throw new EXCEPTION("You need: " + entry.first);
}
...
void StantonIntermediatesUCCSD<F>::calculateOneBodyIntermediates() {
  ...
  (*Fae)["ae"] = (*Fab)["ae"];
  (*Fae)["aa"] += (-1.0) * (*Fab)["aa"];
  if (Fia) { (*Fae)["ae"] += (-0.5) * (*Fia)["me"] * (*Tai)["am"]; }
...
template <typename F>
PTR(Tensor<F>) StantonIntermediatesUCCSD<F>::getRai() {
...
template <typename F>
PTR(Tensor<F>) StantonIntermediatesUCCSD<F>::getRabij() {
