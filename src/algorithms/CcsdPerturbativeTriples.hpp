...
namespace sisi4s {
  ...
  protected:
    ...
    void sliceTensors();
    CTF::Tensor<double> &getSinglesContribution(const Map<3> &);
    CTF::Tensor<double> &getDoublesContribution(const Map<3> &);
    CTF::Tensor<double> &getEnergyDenominator(const Map<3> &);
  };
}
