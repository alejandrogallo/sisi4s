...
struct IntegralProvider {
  ...
  IntegralProvider(size_t no,
      ...
      , Nv(nv)
      , Np(no + nv)
      , chemistNotation(chemistNotation_)
      , unrestricted(unrestricted_) {}

  struct Limit {
    size_t lower, upper, size;
    Limit(size_t l, size_t h)
        : lower(l)
        , upper(h)
        , size(h - l) {}
...
struct VectorIntegralProvider : public IntegralProvider<std::vector<double>> {
  ...
  std::vector<double> compute(Index P, Index Q, Index R, Index S) {
    ...
    const Limit pLim(indexToLimits(P)), qLim(indexToLimits(Q)),
        rLim(indexToLimits(R)), sLim(indexToLimits(S));
      ...
      for (size_t k(0), Inmlk = 0; k < Np; ++k) {
        for (size_t l(0); l < Np; ++l) {
          for (size_t m(0); m < Np; ++m) {
            ...
            const size_t Ipmlk = pLim[p] + m * pLim.size + l * pLim.size * Np
                               + k * pLim.size * Np * Np;
      ...
      for (size_t p(pLim.lower); p < pLim.upper; ++p) {
        for (size_t k(0); k < Np; ++k) {
          for (size_t l(0); l < Np; ++l) {
            ...
            const size_t Iprlk = pLim[p] + rLim[r] * pLim.size
      ...
      for (size_t q(qLim.lower); q < qLim.upper; ++q) {
        for (size_t p(pLim.lower); p < pLim.upper; ++p) {
          ...
          for (size_t k(0); k < Np; ++k) {
            ...
            const size_t Iprqk = pLim[p] + rLim[r] * pLim.size
      ...
      for (size_t r(rLim.lower); r < rLim.upper; ++r) {
        for (size_t q(qLim.lower); q < qLim.upper; ++q) {
          for (size_t p(pLim.lower); p < pLim.upper; ++p) {
            ...
            const size_t IPQRS = pLim[p] + qLim[q] * pLim.size
              ...
              for (size_t l(0); l < Np; ++l) {
                for (size_t m(0); m < Np; ++m) {
                  for (size_t n(0); n < Np; ++n, ++Inmlk) {
                    ...
                    // <p q | r s> = (p r , q s)
                    const int IPQRS =
                        pLim[p] + qLim[q] * pLim.size
                        + rLim[r] * pLim.size * qLim.size
                        + sLim[s] * pLim.size * qLim.size * rLim.size;

                    result[IPQRS] += C[k + s * Np] * C[l + q * Np]
                                   * C[m + r * Np] * C[n + p * Np]
                                   * Vklmn[Inmlk];
...
struct CtfIntegralProvider : public IntegralProvider<Tensor<double>> {
  ...
  Tensor<double> *compute() {
    ...
    LOGGER(1) << "basis Functions " << C.lens[0] << ", orbitals: " << C.lens[1]
              << std::endl;
    std::vector<int> s(4, NS);
    std::vector<int> l(4, C.lens[0]);
    ...
    if (chemistNotation) {
      ...
      */
      ...
      l[0] = C.lens[1];
      auto VIntermedia1 =
          new Tensor<double>(4, l.data(), s.data(), *Sisi4s::world);
      ...
      //      else {
      //        VTransformed = new Tensor<double>(4, l.data(), s.data(),
      //        *Sisi4s::world);
      //
      //        (*VTransformed)["plmn"] = C["kp"] * V["klmn"];
  ...
  Tensor<double> compute(Index P, Index Q, Index R, Index S) {
    ...
    const Limit p(indexToLimits(P)), q(indexToLimits(Q)), r(indexToLimits(R)),
        s(indexToLimits(S));
...
void computeAndExport(Algorithm &a,
  ...
  for (const auto &integral : integralInfos) {
    ...
    a.out.set<Tensor<double> *>(integral.name, stdVectorToTensor(result, lens));
  }
}
...
DEFSTEP(CoulombIntegralsFromRotatedCoulombIntegrals) {
  ...
  if (engineType == EngineInfo::VECTOR_FAST) {
    ...
    SlowVectorIntegralProvider engine(No,
                                      Nv,
                                      chemistNotation,
                                      unrestricted,
                                      *C,
    ...
    CtfIntegralProvider
        engine(No, Nv, chemistNotation, unrestricted, *C, *V, *S);
    computeAndExport(*this, engine, integralInfos);
  }

  EMIT() << YAML::Key << "No" << YAML::Value << No << YAML::Key << "Nv"
         << YAML::Value << Nv << YAML::Key << "engine" << YAML::Value
         << engineType << YAML::Key << "chemist-notation" << YAML::Value
         << chemistNotation;
}
