...
DEFSTEP(HartreeFockFromCoulombIntegrals) {
  ...
  const auto updateHamiltonian = [&] {
    F = H;
    fockMatrix = getFockMatrix(D, *V);
    F += fockMatrix;
  };

  const auto updateEnergy = [&] {
    ehf = 0.0;
    for (size_t i = 0; i < Np; i++)
      for (size_t j = 0; j < Np; j++) ehf += D(i, j) * (H(i, j) + F(i, j));
  ...
  const auto updateDensity = [&] {
    ...
    // solve F C = e S C
    Eigen::GeneralizedSelfAdjointEigenSolver<MatrixColumnMajor> gen_eig_solver(
        F,
        S);
  ...
  do {
    ...
    if (iter == 1) {
      LOGGER(1) << std::setprecision(16) << std::setw(10) << "Iter"
                << "\t"
                << "E"
                << "\t"
                << "DeltaE"
    ...
    LOGGER_IT(1) << iter << "\t" << ehf << "\t" << energyDifference << "\t"
                 << rmsd << "\t" << std::endl;
  ...
  IF_GIVEN("FockMatrix", int pp[] = {(int)Np, (int)Np};
