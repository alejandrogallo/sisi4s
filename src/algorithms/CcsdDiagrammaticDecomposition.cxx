...
void CcsdDiagrammaticDecomposition::run() {
...

  {
    ...
    (*Rabij)["abij"] =
        (1.0) * realDressedGammaGai["Gai"] * realGammaGab["Gbc"] * (*Tai)["cj"];
    (*Rabij)["abij"] +=
        (1.0) * imagDressedGammaGai["Gai"] * imagGammaGab["Gbc"] * (*Tai)["cj"];
  ...
  Tensor<double> Xklij(false, *Vijkl);
  Xklij.set_name("Xklij");
  // we have to remove the singles sinlges term which appears in our abcd
  // contraction
  (*Rai)["ai"] = 0.0;
  Xklij["klij"] = (*Vabij)["cdkl"] * Xabij["cdij"];
...
void CcsdDiagrammaticDecomposition::evaluateEnergy(std::string diagramType,
                                                   Tensor<double> &deltaabij,
                                                   Tensor<double> &deltaai,
                                                   Tensor<double> &Rabij,
                                                   Tensor<double> &Rai) {
  // energy denominator
  CTF::Transform<double, double>(std::function<void(double, double &)>(
      [](double denominator, double &res) {
        res = res / denominator;
      }))(deltaabij["abij"], Rabij["abij"]);
  CTF::Transform<double, double>(std::function<void(double, double &)>(
      [](double edenom, double &res) { res = res / edenom; }))(deltaai["ai"],
                                                               Rai["ai"]);
  ...
  LOG(0, diagramType) << std::setprecision(10) << "energy= " << dire + exce
                      << std::endl;
}

void CcsdDiagrammaticDecomposition::sliceIntoResiduum(Tensor<double> &Rxyij,
                                                      int a,
                                                      int b,
                                                      Tensor<double> &Rabij) {
  int Nx(Rxyij.lens[0]);
