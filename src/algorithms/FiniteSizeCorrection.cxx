...
DEFSPEC(
    ...
    SPEC_IN(
        {"constantFactor", SPEC_VALUE("TODO: DOC", double)},
        {"cutOffRadius", SPEC_VALUE_DEF("TODO: DOC", double, 100)},
        {"shortestGvector", SPEC_VALUE("TODO: DOC", double)},
        {"volume", SPEC_VALUE("TODO: DOC", double)},
        {"FiniteSize", SPEC_VALUE_DEF("TODO: DOC", int64_t, 0)},
        {"fReadFromFile", SPEC_VALUE_DEF("TODO: DOC", int64_t, 0)},
        {"fTricubic", SPEC_VALUE_DEF("TODO: DOC", int64_t, 1)},
        {"gridPointsFibonacci",
         SPEC_VALUE_DEF("TODO: DOC", int64_t, DEFAULT_NUM_FIBONACCI)},
        {"gridPointsInterpolation", SPEC_VALUE_DEF("TODO: DOC", int64_t, 61)},
        ...
        {"ParticleHoleCoulombVertex",
         SPEC_VARIN("TODO: DOC", Tensor<complex> *)},
        {"SinglesAmplitudes", SPEC_VARIN("TODO: DOC", Tensor<complex> *)},
    ...
    SPEC_OUT({"RealLattice", SPEC_VAROUT("TODO: DOC", Tensor<double> *)},
             {"ReciprocalLattice", SPEC_VAROUT("TODO: DOC", Tensor<double> *)},
             {"StructureFactor", SPEC_VAROUT("TODO: DOC", Tensor<double> *)},
             {"StructureFactors", SPEC_VAROUT("TODO: DOC", Tensor<double> *)},
             {"StructureFactort", SPEC_VAROUT("TODO: DOC", Tensor<double> *)}));

IMPLEMENT_ALGORITHM(FiniteSizeCorrection) {
  int fReadFromFile(in.get<int64_t>("fReadFromFile", 0));
  if (fReadFromFile == 1) {
    LOG(0, "FiniteSize") << "Reading structure factor from file" << std::endl;
    readFromFile();
  } else {
    if (in.is_of_type<Tensor<double> *>("DoublesAmplitudes")) {
      LOG(0, "FiniteSize") << "Calculating real structure factor" << std::endl;
      calculateRealStructureFactor();
    } else {
      LOG(0, "FiniteSize") << "Calculating complex structure factor"
                           << std::endl;
      calculateComplexStructureFactor();
    }
  }

  int fFiniteSizeCorrection(in.get<int64_t>("FiniteSize", 0));
...
public:
  ...
  Momentum()
      ...
      , l(0.0)
      , vg(0.) {}
  Momentum(sisi4s::Vector<> v_, double s_ = 0., double vg_ = 0.)
      : v(v_)
      , s(s_)
      , l(v_.length())
      , vg(vg_) {}
...
void FiniteSizeCorrection::readFromFile() {
  ...
  Tensor<double> *realVG(in.get<Tensor<double> *>("CoulombKernel"));
  Tensor<double> *realSG(in.get<Tensor<double> *>("StructureFactor"));
...
void FiniteSizeCorrection::calculateRealStructureFactor() {
  ...
  if (orbitalPairs) {
    LOG(0, "Orbital Pair Analysis")
        << "Treating only pairs " << orbitalPairStart << " to "
  ...
  Tensor<double> *realInfVG(in.get<Tensor<double> *>("CoulombKernel"));
  Tensor<double> *realVG(new Tensor<double>(false, *realInfVG));
  // Define take out inf funciton
  class TakeOutInf {
  public:
    double operator()(double x) { return std::isinf(x) ? 0.0 : x; }
  };
  ...
  class InvSqrt {
  public:
    double operator()(double x) { return std::sqrt(1.0 / x); }
  };
  ...
  Tensor<double> realCGai(3,
                          GammaGai->lens,
                          GammaGai->sym,
                          *GammaGai->wrld,
                          "RealCGai");
  Tensor<double> imagCGai(3,
                          GammaGai->lens,
                          GammaGai->sym,
                          *GammaGai->wrld,
                          "ImagCGai");
  ...
  Tensor<double> realConjCGai(3,
                              GammaGai->lens,
                              GammaGai->sym,
                              *GammaGai->wrld,
                              "RealConjCGai");
  Tensor<double> imagConjCGai(3,
                              GammaGai->lens,
                              GammaGai->sym,
                              *GammaGai->wrld,
                              "ImagConjCGai");
  ...
  if (orbitalPairs) {
    ...
    realTabij =
        new Tensor<double>(3, lens, Tabij->sym, *Tabij->wrld, "realTabij");
    (*realTabij) = Tabij->slice(aiSliceStart, aiSliceEnd);
  } else {
    // Get Tabij
    realTabij = in.get<Tensor<double> *>("DoublesAmplitudes");
  ...
  (*realSGd)["G"] =
      (1.0) * realConjCGai["Gai"] * realCGai["Gbj"] * (*realTabij)["abij"];
  (*realSGd)["G"] +=
      (-1.0) * imagConjCGai["Gai"] * imagCGai["Gbj"] * (*realTabij)["abij"];

  (*realSGx)["G"] +=
      (1.0) * realConjCGai["Gaj"] * realCGai["Gbi"] * (*realTabij)["abij"];
  (*realSGx)["G"] +=
      (-1.0) * imagConjCGai["Gaj"] * imagCGai["Gbi"] * (*realTabij)["abij"];

  (*realSG)["G"] = (2.0) * (*realSGd)["G"] + (-1.0) * (*realSGx)["G"];
  out.set<Tensor<double> *>("StructureFactor", realSG);
...
void FiniteSizeCorrection::calculateComplexStructureFactor() {
  ...
  */
  ...
  (*SG)["G"] =
      (2.0) * conjTransposeCGia["Gia"] * CGia["Gjb"] * (*Tabij)["abij"];
  (*SG)["G"] +=
      (-1.0) * conjTransposeCGia["Gja"] * CGia["Gib"] * (*Tabij)["abij"];
...
void FiniteSizeCorrection::dryCalculateStructureFactor() {
  ...
  if (isArgumentGiven("CoulombVertex")) {
    DryTensor<complex> *GammaFai(in.get<DryTensor<complex> *>("CoulombVertex"));
  ...
  int syms[] = {NS, NS};
  out.set<Tensor<double> *>("StructureFactor",
                            new DryTensor<>(2, len, syms, SOURCE_LOCATION));
}
...
void FiniteSizeCorrection::interpolation3D() {
  Tensor<double> *momenta(in.get<Tensor<double> *>("Momenta"));
  ...
  for (int g(0); g < NG; ++g) { check_grid += cartesianMomenta[g]; }
  LOG(1, "interpolation3D")
      << "Length of the sum of all G vectors=" << check_grid.length()
      << std::endl;
  if (check_grid.length() > 1e-10) {
    LOG(1, "interpolation3D")
        << "Working with half G grid, completing the other half..."
        << std::endl;
    LOG(1, "interpolation3D")
    ...
    cartesianGrid[0] =
        Momentum(cartesianMomenta[0], structureFactors[0], VofG[0]);
    for (int g(1); g < NG; ++g) {
      cartesianGrid[g] =
          Momentum(cartesianMomenta[g], 0.5 * structureFactors[g], VofG[g]);
      cartesianGrid[(g + NG - 1)] = Momentum(cartesianMomenta[g] * (-1.),
                                             0.5 * structureFactors[g],
                                             VofG[g]);
    }
    NG = (2 * NG - 1);
  } else {
    ...
    for (int g(0); g < NG; ++g)
      cartesianGrid[g] =
          Momentum(cartesianMomenta[g], structureFactors[g], VofG[g]);
  }
  ...
  if (isArgumentGiven("shortestGvector")) {
    GC = in.get<double>("shortestGvector");
  } else {
    GC = a.length();
  }
  ...
  auto ctfRealLattice(new Tensor<double>(2, std::vector<int>({3, 3}).data()));

  std::vector<int64_t> indices(ctfReciprocalLattice->wrld->rank == 0 ? 3 * 3
                                                                     : 0);
  for (size_t i(0); i < indices.size(); ++i) { indices[i] = i; }
  ctfReciprocalLattice->write(indices.size(),
                              indices.data(),
                              ReciprocalLattice.data()->coordinate);
  ctfRealLattice->write(indices.size(), indices.data(), T.data()->coordinate);

  if (isArgumentGiven("ReciprocalLattice")) {
    out.set<Tensor<double> *>("ReciprocalLattice", ctfReciprocalLattice);
  }

  if (isArgumentGiven("RealLattice")) {
    out.set<Tensor<double> *>("RealLattice", ctfRealLattice);
  }
  ...
  LOG(2, "FiniteSizeInterpolation")
      << "directMin=" << directMin << ", directMax=" << directMax << std::endl;
  ...
  LOG(2, "FiniteSizeInterpolation")
      << "boxOrigin=" << boxOrigin << " boxDimensions=" << boxDimensions
      << std::endl;
        ...
        for (int dz(-1); dz <= 1; ++dz) {
          for (int dy(-1); dy <= 1; ++dy) {
            for (int dx(-1); dx <= 1; ++dx) {
              int64_t offset(dx
                             + boxDimensions[0] * (dy + boxDimensions[1] * dz));
  ...
  LOG(2, "FiniteSizeInterpolation")
      << "Number of momentum points inside cutoff=" << interiorPointsCount
      << ", Number of momentum points on boundary=" << boundaryPointsCount
      << std::endl;

  // create trilinear or tricubic interpolator
  // TODO: use factory to select different interpolators, similar to mixers
  // int fTricubic(in.get<int64_t>("fTricubic", 1));
  gte::IntpTricubic3<double> interpolatedSG(boxDimensions[0],
                                            boxDimensions[1],
                                            boxDimensions[2],
                                            boxOrigin[0],
                                            1,
  ...
  **/
  // spherically sample
  int64_t N(in.get<int64_t>("gridPointsFibonacci", DEFAULT_NUM_FIBONACCI));
  double lastLength(-1);
  averageSGs.clear();
  GLengths.clear();
  meanErrorSG.clear();
  ...
  //  for (int g(0); g<num; ++g){
  //  LOG(2,"sphericalAv") << GLengths[g] << " " << averageSGs[g] << " " <<
  //  meanErrorSG[g] << std::endl;
  //  }
  ...
  LOG(1, "interpolation3D")
      << "Number of basis vectors of small BZ=" << smallBZ.size() << std::endl;
  for (std::vector<int>::size_type i = 0; i != smallBZ.size(); i++) {
    LOG(2, "interpolation3D")
        << "smallBZ basis vector: " << smallBZ[i] << std::endl;
  }

  // integration in 3D
  double constantFactor(in.get<double>("constantFactor"));
  const int gridPointsInterpolation(
      in.get<int64_t>("gridPointsInterpolation", 61));
  // cutOffRadius is set to a big default value 100 to ensure
  // the integration is over the whole G grid.
  double cutOffRadius(in.get<double>("cutOffRadius", 100));
  LOG(0, "interpolation3D")
      << "constantFactor: " << constantFactor << std::endl;
  LOG(0, "interpolation3D")
      << "gridPointsInterpolation: " << gridPointsInterpolation << std::endl;
  LOG(0, "interpolation3D") << "cutOffRadius: " << cutOffRadius << std::endl;
  const int N0(gridPointsInterpolation), N1(gridPointsInterpolation),
  ...
  MpiCommunicator communicator(Sisi4s::world->rank,
                               Sisi4s::world->np,
                               Sisi4s::world->comm);
  ...
  for (int t0(-N0); t0 <= N0; ++t0) {
    for (int t1(-N1); t1 <= N1; ++t1) {
      for (int t2(-N2); t2 <= N2; ++t2) {
        if ((std::abs(t0 + t1 + t2)) % Sisi4s::world->np
            == Sisi4s::world->rank) {
  ...
  LOG(2, "interpolation3D")
      << "Number of points in summation=" << countNO << std::endl;
}
...
// than 1/2.
bool FiniteSizeCorrection::IsInSmallBZ(Vector<> point,
                                       double scale,
                                       std::vector<Vector<>> smallBZ) {
  ...
  for (std::vector<int>::size_type i = 0; i != smallBZ.size(); i++) {
    ...
    if (abs(abs(smallBZ[i].dot(point)) / smallBZ[i].length()
                / smallBZ[i].length() * scale
            - 1.0)
            < epsilon
        || abs(smallBZ[i].dot(point)) / smallBZ[i].length()
                       / smallBZ[i].length() * scale
                   - 1.0
               < -epsilon) {
      countVector++;
  ...
  if (countVector == smallBZ.size()) { return true; }
  return false;
}
...
double FiniteSizeCorrection::integrate(sisi4s::Inter1D<double> Int1d,
  ...
  for (int i = 0; i < steps; ++i) s += simpson(Int1d, start + h * i, h);
  return h * s;
}
...
double FiniteSizeCorrection::simpson(sisi4s::Inter1D<double> Int1d,
                                     double x,
                                     double h) {
  return (SGxVG(Int1d, x) + 4 * SGxVG(Int1d, x + h / 2.) + SGxVG(Int1d, x + h))
       / 6.;
}

double FiniteSizeCorrection::SGxVG(sisi4s::Inter1D<double> Int1d, double x) {
  return (x > 0. && x < GC) ? (cos(x / GC * M_PI) + 1) * 1. / 2 / x / x
                                  * Int1d.getValue(x) * x * x
                            : 0.;
}

void FiniteSizeCorrection::calculateFiniteSizeCorrection() {
  sisi4s::Inter1D<double> Int1d(GLengths.size(),
                                GLengths.data(),
                                averageSGs.data());
  ...
  for (unsigned int i(0); i < GLengths.size(); ++i) {
    LOG(2, "StructureFactor")
        << GLengths[i] << " " << averageSGs[i] << std::endl;
  }

  int kpoints(in.get<int64_t>("kpoints", 1));
  double volume(in.get<double>("volume"));
  double constantFactor(in.get<double>("constantFactor"));

  double r1 = integrate(Int1d, 0.0, GC, 1000) * constantFactor * volume
            * kpoints * 4 * M_PI;
  double sumSGVG(0.);
  ...
  LOG(0, "FiniteSize") << "Corrected e=" << sumSGVG + inter3D - sum3D
                       << std::endl;

  {
    std::stringstream stream;
    stream << std::setprecision(10)
           << "Summation within cutoff radius=" << sum3D << std::endl;
    LOG(1, "FiniteSize") << stream.str();
  }
  {
    std::stringstream stream;
    stream << std::setprecision(10)
           << "Integral within cutoff radius=" << inter3D << std::endl;
    LOG(1, "FiniteSize") << stream.str();
  }

  {
    std::stringstream stream;
    stream << std::setprecision(10) << "Spherical Averaging Correction=" << r1
           << std::endl;
    LOG(1, "FiniteSize") << stream.str();
  }
  {
    std::stringstream stream;
    stream << std::setprecision(10)
           << "Spherical Averaging Corrected=" << sumSGVG + r1 << std::endl;
    LOG(1, "FiniteSize") << stream.str();
  }
...
void FiniteSizeCorrection::dryCalculateFiniteSizeCorrection() {
  in.get<int64_t>("kpoints", 1);
  in.get<double>("volume");
  in.get<double>("constantFactor");
