...
Data *Algorithm::getArgumentData(std::string const &name) {
  ...
  if (!data) {
    ...
    //    throw new EXCEPTION(std::stringstream() << "Missing data: " <<
    //    dataIterator->second);
    throw new EXCEPTION(sStream.str());
  }
  return data;
...
sisi4s::real Algorithm::getRealArgument(const std::string &name,
...
sisi4s::real Algorithm::getRealArgumentFromInteger(IntegerData *integerData) {
  ...
  if (int64_t(value) != integerData->value) {
    LOG(0, "root")
        << "Warning: loss of precision in conversion from integer to real."
        << std::endl;
  }
  return value;
}
sisi4s::real Algorithm::getRealArgumentFromTensor(TensorData<real> *data) {
  Assert(data->value->order == 0,
...
C *Algorithm::getContainerArgument(std::string const &name) {
  ...
  std::stringstream sStream;
  sStream << "Incompatible type for argument: " << name << ". "
          << "Excpected container of " << TypeTraits<F>::getName() << ", found "
          << data->getTypeName() << ".";
  throw new EXCEPTION(sStream.str());
}

template <typename F, typename C>
void Algorithm::allocateContainerArgument(std::string const &name,
                                          C *container) {
  Data *mentionedData(getArgumentData(name));
  new ContainerData<F, C>(mentionedData->getName(), container);
}
template void
...
template void
Algorithm::allocateContainerArgument<sisi4s::complex,
                                     std::vector<sisi4s::complex>>(
    std::string const &name,
...
T *Algorithm::getTensorArgument(std::string const &name) {
  ...
  std::stringstream sStream;
  sStream << "Incompatible type for argument: " << name << ". "
          << "Excpected tensor of " << TypeTraits<F>::getName() << ", found "
          << data->getTypeName() << ".";
  throw new EXCEPTION(sStream.str());
}
// instantiate
template Tensor<Float64> *
Algorithm::getTensorArgument<Float64, Tensor<Float64>>(std::string const &);
template Tensor<Complex64> *
Algorithm::getTensorArgument<Complex64, Tensor<Complex64>>(std::string const &);
template DryTensor<Float64> *
Algorithm::getTensorArgument<Float64, DryTensor<Float64>>(std::string const &);
template DryTensor<Complex64> *
Algorithm::getTensorArgument<Complex64, DryTensor<Complex64>>(
    std::string const &);
...
// instantiate
template Tensor<Float64> *
Algorithm::getTensorArgumentFromReal<Float64, Tensor<Float64>>(RealData *);
template Tensor<Complex64> *
Algorithm::getTensorArgumentFromReal<Complex64, Tensor<Complex64>>(RealData *);
template DryTensor<Float64> *
Algorithm::getTensorArgumentFromReal<Float64, DryTensor<Float64>>(RealData *);
template DryTensor<Complex64> *
Algorithm::getTensorArgumentFromReal<Complex64, DryTensor<Complex64>>(
    RealData *);

template <typename F, typename T>
void Algorithm::allocatedTensorArgument(std::string const &name, T *tensor) {
  Data *mentionedData(getArgumentData(name));
  new TensorData<F, typename TensorTypeTraits<F, T>::BaseType>(
      mentionedData->getName(),
      tensor);
...
// instantiate
template void Algorithm::allocatedTensorArgument<Float64, Tensor<Float64>>(
    std::string const &name,
    Tensor<Float64> *tensor);
...
template void Algorithm::allocatedTensorArgument<Complex64, Tensor<Complex64>>(
    std::string const &name,
    Tensor<Complex64> *tensor);
// TODO: remove specialized tensors (matrix, vector, scalar)
template void Algorithm::allocatedTensorArgument<sisi4s::complex,
                                                 CTF::Matrix<sisi4s::complex>>(
    std::string const &name,
    CTF::Matrix<sisi4s::complex> *tensor);
template void Algorithm::allocatedTensorArgument<sisi4s::complex,
                                                 CTF::Vector<sisi4s::complex>>(
    std::string const &name,
    CTF::Vector<sisi4s::complex> *tensor);
template void Algorithm::allocatedTensorArgument<sisi4s::complex,
                                                 CTF::Scalar<sisi4s::complex>>(
    std::string const &name,
    CTF::Scalar<sisi4s::complex> *tensor);

template void Algorithm::allocatedTensorArgument<Float64, DryTensor<Float64>>(
    std::string const &name,
    DryTensor<Float64> *tensor);
// TODO: remove specialized tensors (matrix, vector, scalar)
template void Algorithm::allocatedTensorArgument<Float64, DryMatrix<Float64>>(
    std::string const &name,
    DryMatrix<Float64> *tensor);
template void Algorithm::allocatedTensorArgument<Float64, DryVector<Float64>>(
    std::string const &name,
    DryVector<Float64> *tensor);
template void Algorithm::allocatedTensorArgument<Float64, DryScalar<Float64>>(
    std::string const &name,
    DryScalar<Float64> *tensor);

template void
Algorithm::allocatedTensorArgument<Complex64, DryTensor<Complex64>>(
    std::string const &name,
    DryTensor<Complex64> *tensor);
// TODO: remove specialized tensors (matrix, vector, scalar)
template void
Algorithm::allocatedTensorArgument<Complex64, DryMatrix<Complex64>>(
    std::string const &name,
    DryMatrix<Complex64> *tensor);
template void
Algorithm::allocatedTensorArgument<Complex64, DryVector<Complex64>>(
    std::string const &name,
    DryVector<Complex64> *tensor);
template void
Algorithm::allocatedTensorArgument<Complex64, DryScalar<Complex64>>(
    std::string const &name,
    DryScalar<Complex64> *tensor);
