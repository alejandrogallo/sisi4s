...
template <typename F>
IrmlerTensor<F> readBinaryTensorSerial(
  std::string filename, int64_t offset = 0, int64_t elements = 0, bool talk = true
) {
  std::vector<F> data;
  ...
  if (talk) LOG(1, "SerialReader") << "reading tensor from  " << filename << std::endl;
  ...
  for (int i=0; i<header.order; i++) {
    ...
    if (talk) LOG(1, "SerialReader") << "lens " << dimensionHeader.length << std::endl;
...
// FUNCTIONS
double ParenthesisTriples::getEnergy(
  const double epsijk, const double *Tabc_, const double *Zabc_
)
{
  double energy(0.);
  ...
  for (int64_t c=0; c<Nv; c++)
    for (int64_t b=0; b<Nv; b++){
      ...
      for (int64_t a=0; a<Nv; a++){
        ...
        energy += Tabc_[a + Nv*b + Nv*Nv*c] * Zabc_[a + Nv*b + Nv*Nv*c] / denominator;
      }
  }
  return energy;
}

double getEnergyZero(
  const int Nv, const double epsijk,
  const double *epsa, const double *Tabc_, const double *Zabc_
)
{
...
inline void permuteAddOne(
  const int Nv, const double *input, double *output
)
{
    ...
    for (int64_t j(0); j<Nv; j+=blockSize){
      ...
      for (int64_t i(0); i< Nv; i+=blockSize){
        ...
        for (int64_t ii(i); ii < i + inci; ii++){
          output[ii + jj*Nv + k*Nv*Nv] = input[jj + ii*Nv + k*Nv*Nv];
        }
      }
    }
  }
...
inline void fullPermutationZero(
  const int Nv, const double *input, double *output
)
{
  const int64_t blockSize=16;
  ...
  for (int64_t k = (0); k< Nv; k+=blockSize){
    ...
    for (int64_t j(0); j<Nv; j+=blockSize){
      ...
      for (int64_t i(0); i< Nv; i+=blockSize){
        ...
        for (int64_t jj(j); jj < j + incj; jj++)
        for (int64_t ii(i); ii < i + inci; ii++){
          output[ii + jj*Nv + kk*Nv*Nv] = 8.0*input[ii + jj*Nv + kk*Nv*Nv]
...
inline void fullPermutationOne(
  const int Nv, const double *input, double *output
)
{
  const int64_t blockSize=16;
  ...
  for (int64_t k = (0); k< Nv; k+=blockSize){
    ...
    for (int64_t j(0); j<Nv; j+=blockSize){
      ...
      for (int64_t i(0); i< Nv; i+=blockSize){
        ...
        for (int64_t jj(j); jj < j + incj; jj++)
        for (int64_t ii(i); ii < i + inci; ii++){
          output[ii + jj*Nv + kk*Nv*Nv] = 2.0*input[ii + jj*Nv + kk*Nv*Nv]
...
inline void fullPermutationTwo(
  const int Nv, const double *input, double *output
)
{
  const int64_t blockSize=16;
...
)
{
  ...
  if (holeDiagram){
    ...
    dgemm_(
      "N", "N", &blasNvNv, &blasNv, &blasNo, &mOne, scratchV, &blasNvNv,
      &Vhphh[j*No*Nv+k*No*No*Nv], &blasNo, &Zero, output, &blasNvNv
    );
    // t[ablj] * V[lcik] = F[abc]                                           H1
    dgemm_(
      "N", "N", &blasNvNv, &blasNv, &blasNo, &mOne, &Tabij[j*No*Nv*Nv],
      &blasNvNv, &Vhphh[i*No*Nv+k*No*No*Nv], &blasNo, &pOne, output, &blasNvNv
    );
    // t[cali] * V[lbkj] = F[cab] -> T[acl](i)* V[lbkj] = F[acb] (PERM1)    H3
    dgemm_(
      "N", "N", &blasNvNv, &blasNv, &blasNo, &mOne, scratchV, &blasNvNv,
      &Vhphh[k*No*Nv+j*No*No*Nv], &blasNo, &Zero, scratchO, &blasNvNv
    );
    // t[aclk] * V[lbij] = F[acb]                                (PERM1)   H5
    dgemm_(
      "N", "N", &blasNvNv, &blasNv, &blasNo, &mOne, &Tabij[k*No*Nv*Nv],
      &blasNvNv, &Vhphh[i*No*Nv+j*No*No*Nv], &blasNo, &pOne, scratchO, &blasNvNv
    );
    ...
    dgemm_(
      "T", "T", &blasNv, &blasNvNv, &blasNo, &mOne, &Vhphh[k*No*Nv+i*No*No*Nv],
      &blasNo, scratchV, &blasNvNv, &pOne, output, &blasNv
    );
    // t[bclk] * V[laji] = F[bca] -> V[laji] * t[bclk] = F[abc]            H2
     dgemm_(
      "T", "T", &blasNv, &blasNvNv, &blasNo, &mOne, &Vhphh[j*No*Nv+i*No*No*Nv],
      &blasNo, &Tabij[k*No*Nv*Nv], &blasNvNv, &pOne, output, &blasNv
    );
  }
  // PARTICLE CONTRACTION
  if (particleDiagram){
    // when we dont calculate the holeDiagram we have to zero the output/scratch0
    double zOrO = holeDiagram ? 1.0 : 0.0;
    // t[aeij] * V[bcek] = F[abc]                                          P0
    dgemm_(
      "N", "T", &blasNv, &blasNvNv, &blasNv, &pOne, &Tabij[i*Nv*Nv+j*No*Nv*Nv],
      &blasNv, integralContainer.k, &blasNvNv, &zOrO, output, &blasNv
    );
    // t[ceki] * V[abej] = F[cab] -> V[abej] * t[ceki] = F[abc]            P5
    dgemm_(
      "N", "T", &blasNvNv, &blasNv, &blasNv, &pOne, integralContainer.j,
      &blasNvNv, &Tabij[k*Nv*Nv+i*No*Nv*Nv], &blasNv, &pOne, output, &blasNvNv
    );
    // WE NEED A PERMUTED V[abe] (i)
    permuteMoveTwo(Nv, integralContainer.i, scratchV);
    // t[cekj] * V[baei] = F[cba] -> v[abe](i) * t[cekj] = F[abc]          P2
    dgemm_(
      "N", "T", &blasNvNv, &blasNv, &blasNv, &pOne, scratchV,
      &blasNvNv, &Tabij[k*Nv*Nv+j*No*Nv*Nv], &blasNv, &pOne, output, &blasNvNv
    );
    // t[beji] * V[acek] = F[bac] -> V[acek] * t[beji] = F[acb] (PERM1)    P1
    dgemm_(
      "N", "T", &blasNvNv, &blasNv, &blasNv, &pOne, integralContainer.k,
      &blasNvNv, &Tabij[j*Nv*Nv+i*No*Nv*Nv], &blasNv, &zOrO, scratchO, &blasNvNv
    );

    // t[aeik] * V[cbej] = F[acb]                               (PERM1)    P3
    dgemm_(
      "N", "T", &blasNv, &blasNvNv, &blasNv, &pOne, &Tabij[i*Nv*Nv+k*No*Nv*Nv],
      &blasNv, integralContainer.j, &blasNvNv, &pOne, scratchO, &blasNv
    );
    // t[bejk] * V[caei] = F[bca] -> v[ace](i) * t[bejk] = F[acb] (PERM1)  P4
    dgemm_(
      "N", "T", &blasNvNv, &blasNv, &blasNv, &pOne, scratchV,
      &blasNvNv, &Tabij[j*Nv*Nv+k*No*Nv*Nv], &blasNv, &pOne, scratchO, &blasNvNv
    );
  }
...
void ParenthesisTriples::singlesContribution(
...
)
{
  ...
  for (int64_t b=0; b < Nv; b++)
  for (int64_t a=0; a < Nv; a++){
    output[a + b*Nv + c*Nv*Nv] += Tai[a + i*Nv] * Vabij[offjk + b + c*Nv];
    output[a + b*Nv + c*Nv*Nv] += Vabij[offij + a + b*Nv] * Tai[c + k*Nv];
    output[a + b*Nv + c*Nv*Nv] += Vabij[offik + a + c*Nv] * Tai[b + j*Nv]; 
  }
 

}

void ParenthesisTriples::getVpppijkFromVertex(
  const std::array<int64_t,3> &ijk, double *scratchO, double *Vpppijk
)
{
  ...
  dgemm_(
    "T", "N", &blasNvNv, &blasNv, &blasNG, &pOne, realGab,
    &blasNG, &realGai[i*Nv*NG], &blasNG, &Zero, scratchO, &blasNvNv
  );
  dgemm_(
    "T", "N", &blasNvNv, &blasNv, &blasNG, &pOne, imagGab,
    &blasNG, &imagGai[i*Nv*NG], &blasNG, &pOne, scratchO, &blasNvNv
  );
  // We have to write the scratch on the container Vpppijk, however last two indices flipped
  permuteMoveOne(Nv, scratchO, Vpppijk);

  dgemm_(
    "T", "N", &blasNvNv, &blasNv, &blasNG, &pOne, realGab,
    &blasNG, &realGai[j*Nv*NG], &blasNG, &Zero, scratchO, &blasNvNv
  );
  dgemm_(
    "T", "N", &blasNvNv, &blasNv, &blasNG, &pOne, imagGab,
    &blasNG, &imagGai[j*Nv*NG], &blasNG, &pOne, scratchO, &blasNvNv
  );
  // We have to write the scratch on the container Vpppijk, however last two indices flipped
  ...
  dgemm_(
    "T", "N", &blasNvNv, &blasNv, &blasNG, &pOne, realGab,
    &blasNG, &realGai[k*Nv*NG], &blasNG, &Zero, scratchO, &blasNvNv
  );
  dgemm_(
    "T", "N", &blasNvNv, &blasNv, &blasNG, &pOne, imagGab,
    &blasNG, &imagGai[k*Nv*NG], &blasNG, &pOne, scratchO, &blasNvNv
  );
  // We have to write the scratch on the container Vpppijk, however last two indices flipped
...
IJKPointer ParenthesisTriples::getVpppijkOnTheFly(
...
)
{
  ...
  std::set_intersection( ijk.begin(), ijk.end()
                       , vtensor.refs.begin(), vtensor.refs.end()
                       , std::back_inserter(intersection)
                       );

  std::set_difference( vtensor.refs.begin(), vtensor.refs.end()
  ...
  for (const auto& i: ijk) {
    ...
    if (tensorMap[i] == nullptr) {
      ...
      vtensor.tensors[c] = new IrmlerTensor<double>(
...
void ParenthesisTriples::run(){
	...
	auto epsaT(readBinaryTensorSerial<double>(
		getTextArgument("ParticleEigenEnergiesFile", "ParticleEigenEnergies.bin")
  ));
  ...
  auto TaiT(readBinaryTensorSerial<double>(
  	getTextArgument("CcsdSinglesAmplitudesFile","CcsdSinglesAmplitudes.bin")
  ));
	Tai = TaiT.data.data();

  auto TabijT(readBinaryTensorSerial<double>(
  	getTextArgument("CcsdDoublesAmplitudesFile","CcsdDoublesAmplitudes.bin")
  ));
  ...
  if (isArgumentGiven("readPPPH")){
    ...
    VppphT = NEW(IrmlerTensor<double>, readBinaryTensorSerial<double>(
      getTextArgument("PPPHCoulombIntegralsFile","PPPHCoulombIntegrals.bin")
    ));
  ...
  for (int64_t i(0); i < nMessages; i++){
    messages[i] = iMessage*(i+1);
  }
  ...
  for ( int64_t i(0); i < No; i++)
    for ( int64_t j(i); j < No; j++)
      for ( int64_t k(j); k < No; k++){
        if ( i == j && i == k ) continue;
        tuplesList[u][0] = i; tuplesList[u][1] = j; tuplesList[u][2] = k;
        u++;
  }
...
//  for (int64_t i = 0; i < 100; i++){
    ...
    if ( i == messages.back()){
      LOG(1,"Finished") << i << " out of total " << nTuples << " tuples" << std::endl;
      messages.pop_back();
    }
    std::array<int64_t,3> ijk;
    ijk[0]=tuplesList[i][0]; ijk[1]=tuplesList[i][1]; ijk[2]=tuplesList[i][2];
    int distinct(0);
    ...
    Time energyTime;
    {
      ...
      if ( distinct == 0) 
        ...
        tupleEnergy = getEnergyOne(Nv, epsijk, epsa, Tabc, Zabc);
    }
