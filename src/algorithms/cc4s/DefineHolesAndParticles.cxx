...
namespace sisi4s {
  ...
  IMPLEMENT_ALGORITHM(DefineHolesAndParticles) {
    ...
    cc4s::HPHeader
      h = YAML::LoadFile(fileName).as<cc4s::HPHeader>();

    LOG(0, "DefineHolesAndParticles")
      << "fermiEnergy: " << h.fermiEnergy << "\n";

    LOG(0, "DefineHolesAndParticles")
      << "read elements: " << h.energies.size() << "\n";

    const auto
      it = std::partition(h.energies.begin(),
                          h.energies.end(),
                          [&h](double e) {
                            return e <= h.fermiEnergy;
                          });

    const TensorIndex
      No = it - h.energies.begin(),
      Nv = h.energies.size() - No,
      ns = NS;


    auto
      *epsi = new Tensor<double>(1, &ns, &No),
