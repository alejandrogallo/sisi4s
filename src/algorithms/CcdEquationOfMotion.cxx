#include <algorithms/CcdEquationOfMotion.hpp>
#include <tcc/Tcc.hpp>
...
ALGORITHM_REGISTRAR_DEFINITION(CcdEquationOfMotion);

// TODO: Study the requirements to treat the problem with real numbers or
// complex

CcdEquationOfMotion::CcdEquationOfMotion(
...
void CcdEquationOfMotion::run() {
  ...
  LOG(1, "CCD_EOM") << "Nv " << Nv << std::endl;
  LOG(1, "CCD_EOM") << "No " << No << std::endl;
  LOG(1, "CCD_EOM") << "Problem dimension " << totalDimension << std::endl;
  ...
  LOG(1, "CCD_EOM") << "Antisymmetrizing Vpqrs " << std::endl;
  ...
  T *Tabij(getTensorArgument<double, T>("CcdDoublesAmplitudes"));
