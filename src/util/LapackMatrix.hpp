...
public:
  ...
  F &operator()(const int i, const int j) { return values[i + j * rows]; }
   ...
   **/
  const F *getValues() const { return values.data(); }

  /**
   * \brief Returns the pointer to the mutable column major data.
   **/
  F *getValues() { return values.data(); }
  ...
  void write(Tensor<F> &ctfA) const {
    if (ctfA.lens[0] != rows || ctfA.lens[1] != columns) {
      ...
      stream << "Tensor is not of correct shape to receive (" << rows << "x"
             << columns << ") matrix: ";
      std::string join("");
...
LapackMatrix<F> operator*(const LapackMatrix<F> &A, const LapackMatrix<F> &B) {
  if (A.getColumns() != B.getRows()) {
    ...
    stream << "Matrix shapes not compatible for multiplication: ("
           << A.getRows() << "x" << A.getColumns() << ") . (" << B.getRows()
           << "x" << B.getColumns() << ")";
    throw new EXCEPTION(stream.str());
  }
  ...
  for (int i(0); i < A.getRows(); ++i) {
    for (int j(0); j < B.getColumns(); ++j) {
      C(i, j) = 0;
      for (int k(0); k < A.getColumns(); ++k) { C(i, j) += A(i, k) * B(k, j); }
    }
  }
  return C;
}
