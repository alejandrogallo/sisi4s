...
const Str oneOrMore("+"), newline("\\n"), anyChar("."), tab("\\t"), anyOf("*"),
    optional("?"), eof("$"), orOf("|"), bof("^") // begin of line
    ,
...
const std::function<Str(Str)> capture([](const Str &i) {
  return "(" + i + ")";
}),
