...
public:
  ...
  int64_t getSeconds() const { return tv_sec; }
  int64_t getFractions() const { return tv_nsec; }
...
inline std::ostream &operator<<(std::ostream &stream, Time const &t) {
  ...
  time << t.getSeconds() << "." << std::setw(Time::FRACTION_DIGITS)
       << std::setfill('0') << t.getFractions();
