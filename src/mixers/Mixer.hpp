...
public:
   ...
   **/
  virtual void append(const PTR(FockVector<F>) &A,
                      const PTR(FockVector<F>) &R) = 0;
...
public:
  ...
  */
   ...
   */
  static PTR(Mixer<F>) create(std::string const &name, Algorithm *algorithm) {
    auto iterator(getMixerMap()->find(name));
    return iterator != getMixerMap()->end() ? iterator->second(algorithm)
                                            : PTR(Mixer<F>)();
  }
...
protected:
  ...
  static std::map<std::string, std::function<PTR(Mixer<F>)(Algorithm *)>>
      *mixerMap;
  /*
