...
public:
  ...
  Vector() {
    for (int d(0); d < D; ++d) { coordinate[d] = 0; }
  }
  Vector(Vector<F, D> const &v) {
    for (int d(0); d < D; ++d) { coordinate[d] = v.coordinate[d]; }
  }
  ...
  Vector<F, D> &operator*=(const F r) {
    for (int d(0); d < D; ++d) { coordinate[d] *= r; }
    return *this;
  }
  ...
  Vector<F, D> operator/(const F r) const {
    Vector<F, D> t;
    for (int d(0); d < D; ++d) { t.coordinate[d] = coordinate[d] / r; }
    return t;
  }
  ...
  Vector<F, 3> cross(Vector<F, 3> const &v) {
    Vector<F, 3> t;
    t.coordinate[0] =
        coordinate[1] * v.coordinate[2] - coordinate[2] * v.coordinate[1];
    t.coordinate[1] =
        coordinate[2] * v.coordinate[0] - coordinate[0] * v.coordinate[2];
    t.coordinate[2] =
        coordinate[0] * v.coordinate[1] - coordinate[1] * v.coordinate[0];
    return t;
  }
  ...
  F operator[](int d) const { return coordinate[d]; }

  F &operator[](int d) { return coordinate[d]; }

  F at(int d) const { return coordinate[d]; }

  F &at(int d) { return coordinate[d]; }
...
inline std::ostream &operator<<(std::ostream &stream,
  ...
  for (int d(0); d < D - 1; ++d) { stream << v.coordinate[d] << ","; }
