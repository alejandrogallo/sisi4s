...
public:
  ...
  static void allocate(int64_t size, SourceLocation const &location) {
    ...
    if (currentTotalSize > maxTotalSize) {
      ...
      extendingResources.push_back(ExtendingResource(maxTotalSize, location));
      /*
  ...
  static void free(int64_t size) { currentTotalSize -= size; }
...
public:
  ...
  virtual ~DryTensor() { free(); }
  virtual void use() {}

  int64_t getElementsCount() const {
    int64_t elementsCount(1);
    for (int i(0); i < order; ++i) { elementsCount *= lens[i]; }
